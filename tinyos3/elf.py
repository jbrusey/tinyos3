...
class ELFSection:
    ...
    def fromString(self, s):
        ...
        (
            ...
            self.sh_addralign,
            self.sh_entsize,
        ) = struct.unpack(self.Elf32_Shdr, s)

    def __str__(self):
        """pretty print for debug..."""
        return (
            "%s(%s, sh_type=%s, sh_flags=%s, "
...
class ELFProgramHeader:
    ...
    def fromString(self, s):
        ...
        (
            ...
            self.p_flags,
            self.p_align,
        ) = struct.unpack(self.Elf32_Phdr, s)

    def __str__(self):
        """pretty print for debug..."""
        return (
            "%s(p_type=%s, p_offset=0x%04x, p_vaddr=0x%04x, p_paddr=0x%04x, "
...
class ELFObject:
    ...
    def fromFile(self, fileobj):
        ...
        if (
            self.e_ident[0:4] != "\x7fELF"
            and self.e_ident[self.EI_CLASS] != self.ELFCLASS32
            and self.e_ident[self.EI_DATA] != self.ELFDATA2LSB
            and self.e_ident[self.EI_VERSION] != 1
        ):
        ...
        for sectionnum in range(self.e_shnum):
            shdr = (
                fileobj.read(self.e_shentsize)
                + "\0" * struct.calcsize(ELFSection.Elf32_Shdr)
            )[0 : struct.calcsize(ELFSection.Elf32_Shdr)]
    ...
    def getLMA(self, section):
        ...
        for p in self.programmheaders:
            if (
                p.p_paddr != 0
                and p.p_type == ELFProgramHeader.PT_LOAD
                and p.p_vaddr != p.p_paddr
                and p.p_vaddr <= section.sh_addr
                and (p.p_vaddr + p.p_memsz >= section.sh_addr + section.sh_size)
                and (
                    not (
                        section.sh_flags & ELFSection.SHF_ALLOC
                        and section.sh_type != ELFSection.SHT_NOBITS
                    )
                    or (
                        p.p_offset <= section.sh_offset
                        and (
    ...
    def __str__(self):
        """pretty print for debug..."""
        return (
            "%s(self.e_type=%r, self.e_machine=%r, self.e_version=%r, sections=%r)"
            % (
                self.__class__.__name__,
                self.e_type,
                self.e_machine,
